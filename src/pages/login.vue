...
<!--		<img class="login-image" style="width:542px; position: absolute;left:50%;top:40px;margin-left: -271px;" src="../assets/img/login_title.png" alt=""/>-->
		<p style="width:542px; position: absolute;left:50%;top:120px;margin-left: -271px;font-size: 46px;font-family:'Hkxzy';color: #fff;letter-spacing: 8px;text-align: center ">信息发布平台</p>
			...
			<div class="login-content">
				...
				<el-form class="login-from-box" :model="loginForm" :rules="loginRules" ref="loginForm">
					<el-form-item prop="username" :label-width="formLabelWidth">
						<el-input @keyup.native.enter="validUser('loginForm',loginForm)" prefix-icon="el-icon-user-solid" clearable placeholder="用户名" v-model="loginForm.username" autocomplete="off"></el-input>
					</el-form-item>
					<el-form-item style="margin-top: 40px;" prop="password" :label-width="formLabelWidth">
						<el-input @keyup.native.enter="validUser('loginForm',loginForm)" prefix-icon="iconfont icon-mima" show-password placeholder="密码" v-model="loginForm.password" autocomplete="off"></el-input>
					</el-form-item>
				</el-form>
				<div class="ub ub-pj ub-ac rember-pwd" style="margin: 40px 0 30px;">
						<div><el-checkbox v-model="checked">记住密码</el-checkbox></div>
						<div><el-button type="text" @click="showLogin=false">注 册</el-button></div>
				</div>
		...
		<div v-else class="login-box">
			...
			<div class="login-content">
				<div class="login-text" style="color:#05ffff;font-size:24px;padding: 50px 0px 38px;">注 册</div>
				<el-form class="login-from-box" :model="registerForm" :rules="loginRules" ref="registerForm">
					<el-form-item prop="username" :label-width="formLabelWidth">
						<el-input @keyup.native.enter="validUser('registerForm',registerForm)" prefix-icon="el-icon-user-solid" clearable placeholder="用户名" v-model="registerForm.username" autocomplete="off"></el-input>
					</el-form-item>
					<el-form-item style="margin-top: 40px;" prop="password" :label-width="formLabelWidth">
						<el-input @keyup.native.enter="validUser('registerForm',registerForm)" prefix-icon="iconfont icon-mima" show-password placeholder="密码" v-model="registerForm.password" autocomplete="off"></el-input>
					</el-form-item>
					<el-form-item style="margin-top: 40px;" prop="repassword" :label-width="formLabelWidth">
						<el-input @keyup.native.enter="validUser('registerForm',registerForm)" prefix-icon="iconfont icon-mima" show-password placeholder="确认密码" v-model="registerForm.repassword" autocomplete="off"></el-input>
					</el-form-item>
				</el-form>
				<div class="ub ub-pc ub-f1 login-btn">
					<el-button style="width:318px;" type="primary" @click="validRegisterUser('registerForm',registerForm)">注 册</el-button>
				</div>
			</div>
		</div>
...
<script>
	import { login ,register} from '../server/api.js'
	...
	export default {
		...
		methods: {
			...
			validRegisterUser(formName, obj) {
				this.$refs[formName].validate((valid) => {
					if(valid) {
			...
			login_user(obj) {
				...
				login(data).then(res => {
					...
					this.$setsessionStorage('userInfo', res);
//					if(this.checked) {
//						let data = {
